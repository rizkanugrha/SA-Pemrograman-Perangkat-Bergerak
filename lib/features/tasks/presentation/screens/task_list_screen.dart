...
class TaskListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Responsive chip container dengan SingleChildScrollView
          SingleChildScrollView(
            ...
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  // Indikator Mode API (Live/Mock)
                  Center(
                    child: Chip(
                      visualDensity:
                          const VisualDensity(horizontal: -4, vertical: -4),
                      labelPadding: const EdgeInsets.symmetric(horizontal: 4),
                      label: Text(activeModeLabel(),
                          style: const TextStyle(fontSize: 9)),
                    ),
                  ),
                  const SizedBox(width: 4),
                  // Indikator Offline menyala merah jika gagal memuat dari API
  ...
  Widget _body(BuildContext context, TaskProvider provider) {
    ...
    if (visibleTasks.isEmpty) {
      // Pull-to-refresh juga ditambahkan di empty state agar user bisa mencoba sync lagi
    ...
    // Bungkus LayoutBuilder dengan RefreshIndicator
    return RefreshIndicator(
      onRefresh: () => context.read<TaskProvider>().loadTasks(),
      child: LayoutBuilder(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.cloud_off,
                size: 48), // Ikon awan terputus untuk error jaringan
